...
(function() {
    ...
    */
     ...
     * move or copy action
     */
    ws.moveAndCopy = function(actionId) {
        function initActionOpFloater() {
            /*setSelectedValue("actionOpFloater-op", 'move');*/
            setSelectedValue("actionOpFloater-op", 'copy');
            $('#actionOpFloater-title').html(p.getAction(_curActionId).name+"("+actionId+")");
    ...
    ws.actionOpFloaterSelectChanged = function() {
        ...
        for (i = 0; i < n+1; i++) {
            if(i==n){
              $('#actionOpFloater-page').append($("<option/>").attr("value", "10000000").text("所有页面"));
            }else{
              page = pList[i];
    ...
    ws.closeActionOpFloater = function(userConfirm) {
        if (userConfirm) {
            ...
            /*var str=+$('#actionOpFloater-title').val();*/

            var str=document.getElementById('actionOpFloater-title').firstChild.nodeValue;
            var startIndex=str.indexOf("(");
            var lastIndex=str.indexOf(")");
            var oldActionId=str.substring(startIndex+1,lastIndex);

            var oldPageId=null;
            var pageList=p.getPageList(targetMID);
            for(var i=0;i<pageList.length;i++){
                var pageKid=pageList[i];
                var actionLists=pageKid.actionList;
                for(var j=0;j<actionLists.length;j++){
                    if(actionLists[j].id==oldActionId){
                        oldPageId=pageKid.id;
                    }
                }
            }
            ...
            if (targetMID && targetPID) {
                actionOperate(targetMID, targetPID, opType,oldPageId);
            } else {
                alert('请选择目标位置哦~');
                return;
            }
   ...
   function renderModuleTreeList() {
    ...
    function workspaceSearch(key, ev) {
        ...
        function getAHtml(a) {
            ...
            if (_isEditMode) {
                str += "<div class=\"action-info\"><a href\"#\" class='btn btn-default btn-xs' onclick=\"ws.moveAndCopy("+a.id+"); return false;\"><i class='glyphicon glyphicon-random'></i>移动/复制接口</a></div>";
            }
        ...
        function getAInfoHtml(a) {
            /*var head = "<h2 style='margin-top:20px;'>接口详情 <span style='font-size: 14px; color: #999;'>(id: " + a.id
                ...
                + a.id + "); return false;'>",*/
            var head = "<h2 style='margin-top:20px;'>接口详情 <span style='font-size: 14px; color: #999;'>(id: " + a.id
                + ")</span></h2><div class='action-info' href='#' onclick='ws.editA(" + a.id + "); return false;'>",
                body = "",
                foot = "</div>";
          ...
          $.ajax({
            ...
            dataType:"TEXT",
            async:false,
            url:"/mock/getDomainNameActionId.do",
            success:function (data) {
              var request_url=data;
              var str=request_url+a.requestUrl;
              if (a.requestUrl) {
                body += "<div class='item'><b>请求Url </b><a id='AId' href\"#\" onclick=\"ws.jumpUrl(); return false;\">" + str+ "</a></div>";
              }
            }
          });
            /*if (a.requestUrl) {
        ...
        ws.jumpUrl=function(){
            var urlPath= document.getElementById('AId').firstChild.nodeValue;
            window.location.href="http://"+urlPath;
         }
        /**
         ...
         */
        function actionOperate(mid, pid, t,oldPageId) {
         ...
         /* var oldPageId=null;
          $.ajax({
            type:"POST",
            data:{"actionId":curAid},
            dataType:"TEXT",
            async:false,
            url:"/mock/getPageIdByActionId.do",
            success:function (data) {
              oldPageId=data;
            }
          });*/
            if(pid==10000000){
                var pageList=p.getPageList(curMid);
                for(var i=0;i<pageList.length;i++){
                    if(pageList[i].id==oldPageId){
                        continue;
                    }
                  action.pageId =pageList[i].id;
                  if (t === 'move') {
