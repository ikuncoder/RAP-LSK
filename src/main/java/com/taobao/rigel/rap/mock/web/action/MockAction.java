...
public class MockAction extends ActionBase {
    ...
    public String getDomainByPageId(){
        //这里是根据actionId来获取域名的
        String result=mockMgr.getDomainNameByPageId(pageId);
        setContent(result);
        return "json";
    }
    public String getPageIdByActionId(){
        String result=mockMgr.getPageIdByActionId(actionId);
        setContent(result);
        return "json";
    }

    public String getDomainNameActionId(){
        String result=mockMgr.getDomainNameActionId(actionId);
        setContent(result);
        return "json";
    }
    ...
    public String createRule() throws UnsupportedEncodingException {
        /**
         * 思路：这里不能根据域名拿到pageid，因为各个外界的客户端需要根据域名访问
         * 所以这里还是需要前端传入pageId
         */
        HttpServletResponse response = ServletActionContext.getResponse();
        HttpServletRequest request = ServletActionContext.getRequest();
        ...
        /*String url2=request.getScheme()+"://"+ request.getServerName();
        String a=request.getRequestURI();
        System.out.println("createRule() run");
        int pageId=mockMgr.getPageIdBydDomainName(url);
        __id__=pageId;*/
        String callback = getCallback();
        boolean isJSON = false;
